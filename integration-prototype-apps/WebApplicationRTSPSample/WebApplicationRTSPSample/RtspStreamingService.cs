...
namespace WebApplicationRTSPSample
{
	public class RtspStreamingService : IHostedService, IDisposable
	{
		...
		public void StartStream(int channel, int streamType = 0)
		{
			...
			string rtspUrl;
			if (_settings.NvrIp == "test")
				rtspUrl = "rtsp://localhost:8554/live";
			else
				rtspUrl = $"rtsp://{_settings.Username}:{_settings.Password}@{_settings.NvrIp}:{_settings.Port}/unicast/c{channel}/s{streamType}/live";
